...
/// On `wasm32-unknown-unknown`, `SystemTime::now()` panics because the
/// platform has no clock. We fall back to `UNIX_EPOCH` so that the
/// in-memory FS can be used without a real clock.
#[inline]
...
impl FileSystem for MemoryFS {
    ...
    fn open_file(&self, path: &str) -> VfsResult<Box<dyn SeekAndRead + Send>> {
        self.set_access_time(path, wasm_now())?;
